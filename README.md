...
Subpath `@ryuzaki13/react-foundation-ui/floating-windows` экспортирует немодальные
`FloatingWindows` и `FloatingWindow`. Область изолирует геометрию и порядок окон;
приложение владеет составом окон и их содержимым. Поддерживаются pointer/keyboard
перемещение, SSR и необязательное сохранение координат.

Размер области задаётся приложением; отдельного portal и блокировки страницы нет.
Контракт, пример подключения и границы ответственности описаны в
[документации floating-windows](https://github.com/Ryuzaki13/react-foundation-ui/blob/master/src/floating-windows/README.md).
