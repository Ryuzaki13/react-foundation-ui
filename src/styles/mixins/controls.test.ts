...
describe("ui-tone", () => {
	it("использует единые формулы локально для поддержки subtree overrides", () => {
		...
		expect(css).toContain("--ui-tone-fill-hover: color-mix(in srgb, var(--brand-fill) 88%, white)");
		expect(css).toContain("--ui-tone-fill-active: color-mix(in srgb, var(--brand-fill) 76%, white)");
