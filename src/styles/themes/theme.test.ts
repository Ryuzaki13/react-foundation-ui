...
describe("theme", () => {
	...
	it("выводит переиспользуемые производные состояния для каждой схемы", () => {
		...
		for (const tone of tones) {
			...
			expect(css).toContain(`--${tone}-fill-hover: color-mix(in srgb, var(--${tone}-fill) 88%, var(--surface-0))`);
			expect(css).toContain(`--${tone}-fill-active: color-mix(in srgb, var(--${tone}-fill) 76%, var(--surface-0))`);
