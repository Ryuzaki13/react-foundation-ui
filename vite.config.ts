...
const missingPeerDevDependencies = externalPackages.filter(
  (packageName) => packageJson.devDependencies?.[packageName] === undefined,
);

if (missingPeerDevDependencies.length > 0) {
  throw new Error(
    "Peer dependencies must also be present in devDependencies for local build: " +
      missingPeerDevDependencies.join(", "),
  );
}
...
function createDefine(configEnv: { mode: string }): Record<string, string> {
  ...
  return {
    ...
    __PREVIEW__: "false",
    __REACT_QUERY_PERSISTENCE_BUSTER__: JSON.stringify(
      "react-foundation-ui-query-test",
    ),
  };
}
...
export default defineConfig((configEnv) => ({
  ...
  test: {
    ...
    exclude: [
      "src/**/*OData*.{test,spec}.{ts,tsx}",
      "src/select/ODataSelect.test.tsx",
